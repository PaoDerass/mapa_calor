...
def login(
    ...
    user = db.query(Usuario).options(joinedload(Usuario.rol)).filter(Usuario.nombre_usuario == form_data.username).first()
    ...
    permisos_lista = [p.nombre for p in user.rol.permisos] if user.rol and user.rol.permisos else []

    access_token = crear_token_acceso(
        data={
            "sub": str(user.nombre_usuario),
            "role": rol_nombre,
            "permissions": permisos_lista
        },
